...
def detect_conjunctiva_local(pil_image: Image.Image):
    ...
    img_height, img_width = mask.shape; img_area = img_height * img_width
    valid_contours = []
    for c in contours:
        area = cv2.contourArea(c)
        if (area > 0.001 * img_area) and (area < 0.5 * img_area): valid_contours.append(c)
...
def vascularity_features_from_conjunctiva(rgb_u8: np.ndarray) -> dict:
    ...
    return {"vessel_area_fraction": float(mask.sum()) / area, "mean_vesselness": float(vmap.mean()), "p90_vesselness": float(np.percentile(vmap, 90)), "skeleton_len_per_area": float(skel.sum()) / area, "branchpoint_density": float(branches.sum()) / area, "tortuosity_mean": float(np.mean(torts)) if torts else 1.0}
